...
- **Current Sprint Transparency**: Squad view at `/current-sprint` - sprint header with name/ID, summary strip (stories, SP, % done) with a **stuck prompt** when any issue is in progress >24h (link to follow up), status chips, single **sub-task summary** line in the summary card (logged h; missing estimate / no log) linking to the full Sub-task time tracking card, daily completion (with SP), burndown with ideal line + axis labels, scope changes, **Stories in sprint** table with **Reporter** and **Assignee** columns and planned window line, sub-task time tracking (estimate/logged/remaining), assignee or reporter notification message generator for missing sub-task time, dependencies/learnings, stuck tasks card (in progress >24h), previous/next sprint snippet, and sprint tabs (latest to oldest by end date). Board pre-select via `?boardId=` or last-selected board (localStorage); optional `sprintId` for tabbed history.
