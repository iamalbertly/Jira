...
import { calculateWorkDays } from './kpiCalculations.js'; // addKPIColumns unused in preview?
import { retryOnRateLimit, isSystemBusy, recordActivity, discoverBoardsWithCache, discoverFieldsWithCache } from './server-utils.js';
import { buildPreviewCacheKey, findBestPreviewCacheSubset, computeRecentSplitConfig } from './preview-helpers.js';
import { discoverFields } from './discovery.js';
...
export async function previewHandler(req, res) {
    ...
    const cacheKey = req.query.bypassCache === 'true' ? `preview:${Date.now()}` : null; // Temp key if bypassing logic fails early? No, will build properly.

    // We need to build the cache key early, but we need validated params first.
    ...
    try {
        ...
        const realCacheKey = buildPreviewCacheKey({
        ...
        let cachedEntry = !bypassCache ? cache.get(realCacheKey) : null;
        let cachedFromBestAvailableSubset = false;
        let cachedKeyUsed = realCacheKey;

        if (!cachedEntry && !bypassCache && preferCacheBestAvailable) {
            const best = findBestPreviewCacheSubset({ selectedProjects, windowStart, windowEnd, maxAgeMs: BEST_AVAILABLE_CACHE_MAX_AGE_MS });
            if (best) {
        ...
        if (cachedEntry) {
            ...
            // ... hydrate field inventory if needed ... (abbreviating hydration logic for safety, assuming it's in the value or we fetch it)
            // For robustness:
            if (!val.meta) val.meta = {};
            val.meta.fromCache = true;
        ...
        for (let i = 0; i < sprintIds.length; i += 3) {
            ...
            const res = await Promise.all(chunk.map(async (sid) => {
                ...
                if (!isRecent && shouldSplitByRecent) {
                    ...
                    const cParams = readCachedSprintIssues(sKey);
        ...
        cache.set(realCacheKey, payload, CACHE_TTL.PREVIEW);
        res.json(payload);
        ...
        if (isPreviewOwner && ownerResolve) {
            ownerResolve();
            inFlightPreviews.delete(req.query.bypassCache === 'true' ? `preview:${Date.now()}` : buildPreviewCacheKey({
                selectedProjects: req.query.projects ? req.query.projects.split(',') : ['MPSA', 'MAS'],
                windowStart: req.query.start || DEFAULT_WINDOW_START,
                windowEnd: req.query.end || DEFAULT_WINDOW_END,
                includeStoryPoints: req.query.includeStoryPoints !== 'false',
                requireResolvedBySprintEnd: req.query.requireResolvedBySprintEnd === 'true',
                includeBugsForRework: req.query.includeBugsForRework !== 'false',
                includePredictability: req.query.includePredictability === 'true',
                predictabilityMode: req.query.predictabilityMode || 'approx',
                includeEpicTTM: req.query.includeEpicTTM !== 'false',
                includeActiveOrMissingEndDateSprints: req.query.includeActiveOrMissingEndDateSprints === 'true'
            }));
        }
    }
}
