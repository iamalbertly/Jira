...
export function buildSprintIssuesCacheKey({
  ...
  const projects = Array.isArray(selectedProjects) ? [...selectedProjects].sort().join(',') : '';
  const ebmFields = fieldIds?.ebmFieldIds || {};
  return `sprintIssues:${sprintId}:${projects}:${requireResolvedBySprintEnd}:${sprintEndDate || ''}:${allowedIssueTypes.join(',')}:${includeSubtaskTotals}:${JSON.stringify(ebmFields)}`;
}

export function readCachedSprintIssues(cacheKey) {
...
export async function fetchSprintIssuesForTransparency(sprintId, agileClient, selectedProjects, allowedIssueTypes = ['Story', 'Bug'], fieldIds = null) {
  const cacheKey = `sprintIssuesTransparency:${sprintId}:${[...selectedProjects].sort().join(',')}:${allowedIssueTypes.join(',')}:${JSON.stringify(fieldIds?.ebmFieldIds || {})}`;
  const cached = cache.get(cacheKey);
  if (cached?.value) {
