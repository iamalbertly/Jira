...
export function calculateMaturityLevel(predictability, velocityConsistency, reworkRatio) {
  // Normalize inputs (assume high predictability, high consistency, low rework = mature)
  const predictabilityScore = predictability || 0;
  const consistencyScore = velocityConsistency || 0;
  const reworkScore = 100 - (reworkRatio || 0); // Invert rework (low rework = good)
  
  // Weighted average
  const maturityScore = (predictabilityScore * 0.4) + (consistencyScore * 0.3) + (reworkScore * 0.3);
