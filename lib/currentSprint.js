...
function resolveRecentSprints(sprints, currentSprint, maxItems = 6) {
  ...
  const list = [];
  if (currentSprint) list.push(currentSprint);
  const active = sprints.find(s => (s.state || '').toLowerCase() === 'active');
  if (active && (!currentSprint || active.id !== currentSprint.id)) list.push(active);
  ...
  for (const sprint of closed) {
    ...
    if (list.find(item => item.id === sprint.id)) continue;
    list.push(sprint);
  }
  return list.map(s => ({
    id: s.id,
